...
async function run(): Promise<void> {
  try {
    ...
    const version_manifest_response_body: string = await version_manifest_response.readBody();
    const versions_manifest: VersionManifest = JSON.parse(version_manifest_response_body);

    const bds_zip = await tc.downloadTool(versions_manifest.download_url);
    const bdsExtractedPath = await tc.extractZip(bds_zip, './bds');

    const bdsCachedPath = await tc.cacheDir(bdsExtractedPath, 'bds', target_version);
